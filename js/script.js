...
function updateGameTime(newTime) {
  gameTime = newTime * 1000; // Update both gameTime and window.gameTime
  window.gameTime = gameTime;
  document.getElementById("info").innerHTML = newTime + "";
}
...
function newGame() {
  ...
  document.getElementById("info").innerHTML = gameTime / 1000 + ""; // Use updated gameTime
  window.timer = null;
}
...
function updateHighScore() {
  ...
  const highScoreValue =
    highScore && !isNaN(highScore) && highScore > 0 ? highScore : "not yet set";
  document.getElementById("highScoreValue").textContent = highScoreValue;
}

function playSound(key) {
  const audio = new Audio(key === " " ? "audio/space.mp3" : "audio/key.mp3");
  audio.volume = 0.2;
  audio.play();
}
...
document
  .getElementById("timeLimitSelect")
  .addEventListener("change", (event) => {
    updateGameTime(event.target.value);
  });

document.addEventListener("DOMContentLoaded", () => {
  updateHighScore();
  updateGameTime(30); // Default to 30 seconds
});
